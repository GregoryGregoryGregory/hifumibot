      ...
      if (commands[cmd].level === 'master') {
        if (Config.permissions.master.indexOf(c.message.author.id) > -1) {
          try {
            ...
            c.message.channel.sendMessage('Oh no! I ran into an error. Please report this to my owner or wait for a further fix. \n```' + e + '```')
            Logger.error(`Command error, thrown by ${commands[cmd].name}: ${e}`)
          }
        } else {
          c.message.channel.sendMessage('This command is only for my senpai.')
          ...
          if (y !== true) {
            datacontrol.customize.reply(c.message, 'timeout').then((x) => {
              if (x === 'default') {
                ...
                c.message.channel.sendMessage(`Espera ${Math.round(y)} segundos más antes de usar eso de nuevo.`)
              } else {
              ...
              if (r >= commands[cmd].level) {
                    ...
                    if (q) {
                      ...
                      datacontrol.customize.reply(c.message, 'nsfw').then((d) => {
                        if (d === 'default') {
                          ...
                          c.message.channel.sendMessage('No estoy permitiendo comandos +18 en este canal.')
                        } else {
                ...
                datacontrol.customize.reply(c.message, 'permissions').then((u) => {
                  if (u === 'default') {
                      ...
                      var reason = (r > 4) ? '**This is a master user only command**, only my senpai can use this command.' : 'If you want to level up, then ask to the server owner.'
                      c.message.channel.sendMessage('Sorry, I can\'t let you use this command!\nLevel required: ' + commands[cmd].level + ', you have level ' + r + '\n' + reason)
                    }
                  } else if (u === 'spanish') {
                       if (r > -1 && !commands[cmd].hidden) {
                      var reason = (r > 4) ? '**Este es un comando para usuarios maestros**, solo mi senpai puede usar este comando.' : 'Si quieres subir de nivel, entonces pregúntale al dueño del servidor.'
                      c.message.channel.sendMessage('Lo siento, ¡no puedo permitirte usar este comando!\nNivel necesario: ' + commands[cmd].level + ', tienes el nivel ' + r + '\n' + reason)
                    } 
				  } else {
        ...
        if (commands[cmd].noDM) {
          c.message.channel.sendMessage('Sorry, I can\'t use this command in DMs. Please invite me to a server and try again.')
          return
        }
        datacontrol.permissions.checkLevel(c.message, c.message.author.id, []).then(function (r) {
          if (r >= commands[cmd].level) {
            ...
            c.message.channel.sendMessage('Sorry, you can\'t run this command in DM.')
          }
        }).catch(function (e) {
          Logger.error('Permission error: ' + e)
        })
...
bot.Dispatcher.on(Event.MESSAGE_CREATE, e => {
  if (e.message.content == "(╯°□°）╯︵ ┻━┻")
    e.message.channel.sendMessage('Whoa! Calm down! ┬─┬﻿ ノ( ゜-゜ノ)')
})
...
bot.Dispatcher.on(Event.DISCONNECTED, function (e) {
  ...
  if (!restarted) {
    restarted = true
    Logger.info('Trying to recover my powers')
    start()
  } else {
    Logger.warn('I give up')
    process.exit(0)
  }
})
